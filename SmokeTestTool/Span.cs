...
namespace SmokeTestTool
{
    class Span
    {
        ...
        public List<string> allSourceSpans { get; set; }
        public void GetAllSourceSpans(string urlAddress, ref TreeNode Result)
        {
        ...
        private void FetchSourceLinksFromSource(string htmlSource, ref TreeNode Result)
        {
            ...
            List<string> listOfSpans = htmlSource.ExtractFromString("<span>", "</span>");

            foreach (var href in listOfSpans)
            {
                TreeNode node = new TreeNode(href);
                Result.Nodes.Add(node);
                allSourceSpans.Add(href);
            }
